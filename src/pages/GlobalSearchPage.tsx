// src/pages/GlobalSearchPage.tsx - Fixed TypeScript errors
...
import { SearchResult, SearchFilters, SerialNumberItem } from '../types';
  ...
  const debouncedSearch = useCallback(
    debounce(async (term: string) => {
      ...
      try {
        ...
        // For now, use empty array for serial numbers since the method doesn't exist
        const serialNumbers: SerialNumberItem[] = [];
        ...
        const results: SearchResult[] = [
          ...
          ...delivered.map(del => ({
            ...
            title: `Delivered ${del.sku || del.serialNumber || 'Item'}`,
            subtitle: del.productName || del.serialNumber || del.customerInfo?.name || 'Unknown Product',
            status: 'Delivered',
          ...
          // Serial numbers section - currently empty but ready for future implementation
          ...serialNumbers.map((sn: SerialNumberItem) => ({
            id: sn.id,
            type: 'serial_number' as const,
            title: `Serial ${sn.serialNumber}`,
            subtitle: `${sn.batchId} • Status: ${sn.status}`,
            status: sn.status,
            date: sn.createdAt || new Date().toISOString(),
            relevanceScore: calculateRelevance(term, (sn.serialNumber || '') + ' ' + sn.batchId),
  ...
  const handleResultClick = (result: SearchResult) => {
    switch (result.type) {
      ...
      case 'delivered':
        ...
        break;
      case 'serial_number':
        navigate(`/search?serial=${result.serialNumber}`);
        break;
    }
  };
